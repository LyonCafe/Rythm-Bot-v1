...
const { getStations } = require('radio-browser')
  ...
  run: async(client,message,args) => {
    ...
    getStations({
			...
			.then(async data => {
				if (!data[0]) return message.channel.send('<:cross8:908666926216134717> No radio found with that name');
				...
				try {
					...
					return message.reply('<:cross8:908666926216134717> You didn\'t choose a song in time.');
				}
				...
				if (index < 0 || index > max - 1) return message.reply(`<:cross8:908666926216134717> The number you provided was too small or too big (1-${max}).`);
