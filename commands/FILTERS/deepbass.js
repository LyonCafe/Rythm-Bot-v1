...
const player = message.client.manager.players.get(message.guild.id)
  ...
  if (!channel) {
   ...
   if (channel.id !== player.voiceChannel) {
    let embed = new MessageEmbed()
      .setColor("GREEN")
      .setDescription(`<:cross8:908666926216134717> must join be in same voice  channel`)
    return message.channel.send(embed)
  }
...
player.setEQ([{
        ...
        const msg = await message.channel.send(` <a:load:908666726529531944> Turning on **deepbass**.`);
		...
		player.clearEffects()
		const msg = await message.channel.send(` <a:load:908666726529531944> Turning off **deepbass**.`);
			const embed = new MessageEmbed()
