...
if (!args[0]) {
			...
			const msg = await message.channel.send(`<a:load:908666726529531944> Turning on **soft**.`);
	...
	player.setEQ(Array(13).fill(0).map((n, i) => ({
        ...
        const msg = await message.channel.send(`<a:load:908666726529531944> Turning off **soft mode**.`);
			const embed = new MessageEmbed()
